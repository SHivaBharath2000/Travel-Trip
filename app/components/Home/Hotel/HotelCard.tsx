...
const HotelCard = ({ hotel,index }: Props) => {
    const [clickInd,setClickInd]=useState(-1)
  return (
    <div>
      <div className="relative h-[300px] w-full rounded-lg cursor-pointer group overflow-hidden">
        <div className="absolute top-4 right-4 z-20 w-8 h-8 bg-white rounded-full text-black flex items-center justify-center flex-col">
          <FaHeart onClick={()=>setClickInd(clickInd===index?-1:index)} className={`w-3 h-3 ${index==clickInd?'text-red-800':'text-gray-400'}`} />
        </div>
